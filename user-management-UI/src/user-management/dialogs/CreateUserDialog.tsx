...
export function CreateUserDialog({ open, handleCloseDialog }: Props) {
  ...
  const canCloseDialog = isSubmitted && !error && !isCreatingUser;
    ...
    >
      ...
      {shouldShowError && (
        <Alert severity="error" className="m-6 mt-2">
          Cannot create this user. Please try again.
        </Alert>
      )}
    </Dialog>
  );
